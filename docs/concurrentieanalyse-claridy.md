...
1. **Withdrawn: the hard boundary statement.** Earlier drafts argued for stating that a company on AFAS, NetSuite or Dynamics is not ours. That recommendation is dropped. Twinfield, Microsoft Dynamics, SAP Business ByDesign and NetSuite are on the roadmap, so writing those platforms into a published disqualification would have to be walked back later, and a boundary that moves is worse than no boundary. The roadmap itself stays off the site until it exists. What remains true is the underlying point about the founder call being the most expensive thing a two-person firm sells, so qualification has to happen somewhere: in the booking flow rather than in published copy.
2. **Thin third-party-verifiable proof.** Our trust surface was always real and founder-led: named founders, photographs, roles, direct mailboxes, a phone number and a full KVK block. That is stronger than six anonymous logos and should never have been described as an absence. The narrower gap is proof a stranger can check without meeting us. Two credentials do exist, Exact Online Premium Partner and Microsoft AI Cloud Partner, and both were briefly published on the About page and in `llms.txt`. They were removed again on 15 August: as a labelled block of two cards they read as filler rather than as evidence, which is a fair judgement. The credentials are real, so the open question is presentation rather than substance. A single line in the About intro or in the footer would carry them without announcing them. What is still missing either way is customer-side proof: no named client, no case study, no quantified outcome from a real engagement. Claridy has six logos with nothing behind them, so that bar is low.
